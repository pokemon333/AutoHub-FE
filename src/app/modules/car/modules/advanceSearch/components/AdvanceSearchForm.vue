<template>
    <div class="pt-4 pb-4  lg:h-full lg:px-14 md:pl-20 md:pr-32  h-[75%] overflow-y-auto ">
        ...
        <div class=" w-1/2">
            <select @change="mainFilterChange" name="type"
                class="w-full border bg-slate-300 mt-2  border-primary-100   peer h-10 rounded-md px-3">
                <option disabled selected>Dealer or Brand</option>
...
let dealerChecked = () => {
    ...
    dealerWithBrand.value.forEach((dealer) => {
        if (checkedDealers.value.includes(dealer.name)) {
            dealer.brand.forEach((brand) => {
                uniqueBrandNames.add(brand.name);
            });
        }
    });
    filteredBrands.value = Array.from(uniqueBrandNames).map((name) => ({ name }));
}
